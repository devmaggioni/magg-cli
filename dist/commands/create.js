...
async function create(program) {
    program
        ...
        try {
            ...
            spinner.fail("O `curl` não está instalado. Instale antes de continuar.");
            return;
        }
  ...
  `;
        try {
            ...
            spinner.succeed(`${answers.option} criado com sucesso em: ${dirName.name}`);
        }
        catch (err) {
            spinner.fail("Erro ao criar o projeto: " + err.message);
        }
    });
}
